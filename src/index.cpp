...
namespace {
  ...
  diskann::Distance<float> *get_distance_function(diskann::Metric m) {
    if (m == diskann::Metric::FAST_L2) {
      ...
      stream << "Only L2 and inner product distance function supported"
             << std::endl;
...
namespace diskann {
  ...
  void Index<T, TagT>::save(const char *filename) {
    ...
    try {
      ...
      throw new FileException(std::string(filename), e);
    }
    ...
    try {
      ...
      if (_final_graph.size() != _nd) {
        ...
        message << "ERROR. mismatch in number of points. Graph has "
                ...
                << _nd << " points. " << std::endl;
        throw new ANNException(message.str(), -1, __FUNCSIG__, __FILE__,
                               __LINE__);
      }
      ...
      try {
        ...
        if (id != _nd)
          throw new ANNException(
              std::string("Error: number of tags differs from the index size"),
              -1, __FUNCSIG__, __FILE__, __LINE__);
      } catch (std::system_error &e) {
        throw new FileException(tag_filename_s, e);
      }
    }
  }
